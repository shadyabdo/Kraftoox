...
export default function YouTubeDownloader() {
  ...
  const handleOpenTool = () => {
    if (!url.trim()) {
      window.open("https://snapscooper.com/tools/youtube-v3", "_blank", "noopener,noreferrer");
    } else {
      window.open(`https://snapscooper.com/tools/youtube-v3?url=${encodeURIComponent(url)}`, "_blank", "noopener,noreferrer");
    }
  };
  ...
  return (
    <ToolShell tool={TOOL}>
      <div className="card p-6">
        <div className="mb-6">
          ...
          <div className="flex gap-3">
            <input
              ...
              onKeyDown={(e) => e.key === "Enter" && handleOpenTool()}
        ...
        <div className="rounded-xl border border-[var(--line)] bg-[var(--surface2)] p-8 text-center">
          ...
          <p className="c-muted text-sm mb-4">
            {isAr
              ? "الصق رابط الفيديو واضغط تنزيل - ستفتح أداة التنزيل في نافذة جديدة"
              : "Paste the video URL and click Download - the download tool will open in a new window"}
          </p>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="c-muted text-xs">
                  {isAr 
                    ? "ستفتح أداة التنزيل في نافذة جديدة مع الرابط جاهزاً"
                    : "The download tool will open in a new window with the URL ready"}
                </p>
              </div>
            </div>
            <div className="flex gap-3">
      ...
      <div className="mt-6">
        <InfoNote>
          {isAr
            ? "نستخدم خدمة Snapscooper لتنزيل الفيديوهات. تأكد من احترام حقوق الملكية الفكرية وعدم تنزيل محتوى محمي بدون إذن."
            : "We use Snapscooper service to download videos. Please respect copyright laws and do not download protected content without permission."}
        </InfoNote>
      </div>
    </ToolShell>
  );
